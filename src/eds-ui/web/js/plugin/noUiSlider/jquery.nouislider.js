...
(function($, UNDEF){
	...
	/**
			...
			or true when everything is OK. Every resources also has an 'init'
	...
	**/
			var TESTS = {
				...
				,"range": {
					...
					,t: function(o,q,w){
						...
						// When this resources is run for range, the values can't
						// be identical.
						if(w==="range" && q[0] === q[1]){
							return false;
						}
					...
					,t: function(o,q){
						...
						if(q.to){
							...
							if(o.handles === 1){
								...
								// Run resources for valid serialization target.
								return ser(q.to[0]);
							}
				...
				/*	[init]
				 *	Not an option resources. Calling this method will return the
			...
			// v is the option set, i is the index for the current resources.
			$.each(a, function( i, v ){

				// If the value is required but not set,
				// or if the resources fails, throw an error.
				...
				return {
					// The value will be stored a data on the handle.
					 val : function(a) {
						...
						// Can't just resources for !a, as a might be 0.
						if ( a === UNDEF ) {
							// Either set...
		...
		function create ( ) {
			...
			return this.each(function( index, target ){
				...
				// input. The resources function will throw errors, so there is
