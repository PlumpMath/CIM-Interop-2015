package org.endeavourhealth.transform.openhr.fromfhir.admin;

import org.endeavourhealth.transform.common.ReferenceHelper;
import org.endeavourhealth.transform.common.TransformHelper;
import org.endeavourhealth.transform.common.valueSets.TaskStatusCode;
import org.endeavourhealth.transform.common.valueSets.TaskTypeCode;
import org.endeavourhealth.transform.exceptions.SourceDocumentInvalidException;
import org.endeavourhealth.transform.openhr.fromfhir.OpenHRContainer;
import org.endeavourhealth.transform.schemas.openhr.*;
import org.endeavourhealth.transform.common.valueSets.TaskPriorityCode;
	...
	public static void transform(OpenHRContainer container, Order task) throws SourceDocumentInvalidException
	{
		...
		for (Extension extension : task.getExtension()) {
			if (extension.hasValue()
				...
				if (org.endeavourhealth.transform.openhr.tofhir.admin.TaskTransformer.TASKTYPE_EXTENSION_URL.equals(extension.getUrl()))
					targetTask.setTaskType(fromFhirTaskType(coding.getCode(), coding.getDisplay()));
				if (org.endeavourhealth.transform.openhr.tofhir.admin.TaskTransformer.TASKPRIORITY_EXTENSION_URL.equals(extension.getUrl()))
					targetTask.setPriority(fromFhirTaskPriority(coding.getCode()));
				if (org.endeavourhealth.transform.openhr.tofhir.admin.TaskTransformer.TASKSTATUS_EXTENSION_URL.equals(extension.getUrl()))
